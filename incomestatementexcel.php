...
<div class="panel"><div class="header">
  ...
  <td style="text-align: right; font-weight: bold; width: 1px; white-space: nowrap">
                            ...
                            <table cellpadding="0" border="1" cellspacing="0" border="0" class="table table-striped table-bordered table-list" id="example">
...
$sel=mysqli_query($con,"select amount as amount,source,date from income where date>='$date1' and date<='$date2' and coopid='$_SESSION[coopid]'  order by date asc ") or die(mysqli_error($con));
   echo"<thead> <tr>
...
<div style="text-align: center; color: #ccc; font-size: 12px; text-shadow: 1px 1px 0px #fff">     
  ...
  <div class="panel"><div class="header"><div class="balance">
      <?php 
								  ...
								  $user_query=mysqli_query($con,"select amount as value from expenses where duedate>='$date1' and duedate<='$date2' and coopid='$_SESSION[coopid]'")or die(mysql_error($con));
								  $totexpenses=0;
									while($row=mysqli_fetch_array($user_query)){
									$value=$row['value'];
									$totexpenses+=$value;
...
$sel=mysqli_query($con,"select reason,amount as amount,account,duedate from expenses where duedate>='$date1' and duedate<='$date2' and coopid='$_SESSION[coopid]' order by duedate asc ") or die(mysqli_error($con));
   echo"<thead> <tr>
