...
$subcat=$_POST['subcat'];
?>
<div style="padding-bottom: 10px"><div style="text-align: center; color: #ccc; font-size: 14px; font-weight: bold; text-shadow: 1px 1px 0px #fff">BANK TRIAL BALANCE 
<?php
                        		include('dbcon.php');
 ...
 echo" on $bank Account: $subcat From $date1 UNTIL $date2"; echo"<a href='bankdebitcredit.php?bank=$bank&&account=$subcat&&date1=$date1&&date2=$date2'>IN DEEP</A>";?></div>
...
<div class="panel"><div class="header">
 ...
 $user_query=mysqli_query($con,"select * from bank WHERE date>='$date1' and date<='$date2' and action='debit' and bankname='$bank' and account='$subcat'")or die(mysqli_error($con));
								  $totincome=0;
 ...
 $user_query=mysqli_query($con,"select * from bank WHERE date>='$date1' and date<='$date2' and action='debit' and bankname='$bank' and account='$subcat'")or die(mysqli_error($con));
   ...
   <th align=center>Amount</td><th align=center>Proof</td><th align=center>Slip</td>
   </TR></thead><tbody>";
$totincome=$i=0;
while($row=mysqli_fetch_array( $user_query))
{
...
<td align=center> $i</td><td align=center> $date</td><td align=center> $particular</td><td align=center> $source</td><td align=center> $amount</td><td align=center> $proof</td><td align=center> $slip</td>

</tr>");
}
$totincome1=$totincome;
$totincome=number_format($totincome);
echo"<tr><th colspan=4>TOTAL</TD><TH>$totincome</th></tr>";
?>									
...
<div style="text-align: center; color: #ccc; font-size: 12px; text-shadow: 1px 1px 0px #fff">     
 ...
 $user_query=mysqli_query($con,"select * from bank WHERE date>='$date1' and date<='$date2' and action='credit' and bankname='$bank' and account='$subcat'")or die(mysqli_error($con));
								  $totexpenses=0;
									while($row=mysqli_fetch_array($user_query)){
									$value=$row['amount'];
									$totexpenses+=$value;
...
$totexpenses=number_format($totexpenses);
echo"<tr><th colspan=4>TOTAL</TD><TH>$totexpenses</th></tr>";
