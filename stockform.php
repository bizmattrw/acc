...
<div class="panel panel-default">
    <div class="panel-heading"><span class="header">Stock Report Form</span></div>
<div class="panel-body" style="background-color: #f9f9f9; box-shadow: inset 0px 1px 0px #fff; padding: 30px">
<FORM action="stockreport.php" method="get" target="_blank">
...
<td>
                        <input name="date2" type="text" class="form-control input-sm" id="date" style="width: 100px; margin-bottom: 0px; text-align: center"  title=" date " 
						 onclick="ds_sh(this);"  placeholder="Until"  required />
						      						...
						      						Choose Item<select name="item" class="form-control input-sm" required >
						<option></option>
                                  <?php 
								  $year=date("Y");
								include('dbcon.php');
								  
								  $user_query1=mysqli_query($con,"select distinct item from stock ")or die(mysqli_error($con));
									while($row1=mysqli_fetch_array($user_query1)){
									$account1=$row1['item']; 
									echo"<option>$account1</option>";
									}
									?>
