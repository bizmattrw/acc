...
$date1=$_POST['date1'];
$date2=$_POST['date2'];
echo"EXPORT TO EXCEL <a href='liabilitiesreportexcel.php?date1=$date1&&date2=$date2'><img src='images/xls.png'>";?>
...
?>
<table style="padding: 30px"><thead><tr><th style="font-weight: bold; font-size: 16px; text-align: center; padding-bottom: 10px" colspan="2"><?php echo"Imyenda Koperative";?></th></tr>
<tr><th style="font-weight: bold; font-size: 24px; text-align: center; padding-bottom: 10px" colspan="2"><?php echo"Ifitiye Abandi";?></th></tr>
...
$sel=mysqli_query($con,"select sum(amount) as amount,source from liabilities where date<='$date2' and amount>0 and source!='Add profit' and source!='LONGTERM LOAN' and source!='Less loss'
 and source!='Capital' group by source order by source asc ") or die(mysqli_error($con));
$totcredit=$totampay=0;
   echo"<thead> <tr>
   <th style='padding: 3px; padding-left: 25px; padding-left: 5px; border-top-width: 1px; border-bottom-width: 1px; font-weight: bold'>Abayatanze + Impamvu</td> 
...
$amount=$med['amount'];
$totcredit+=$amount;
$sel1=mysqli_query($con,"select sum(amount) as ampay from levelscaisse where liability='$ideni' and date<='$date2'")or die(mysqli_error($con));
$med1=mysqli_fetch_array($sel1);
$ampay=$med1['ampay'];
$totampay+=$ampay;
//$date=$med['date'];
$amountf=number_format($amount);
$ampayf=number_format($ampay);
//$amremf=number_format($amrem);
print("<tr bgcolor=white>
<td style='padding: 1px; padding-left: 25px; padding-left: 5px; border-top-width: 1px; border-bottom-width: 1px; font-weight: bold'> <a HREF='liabilitiespay.php?date1=$date1&&date2=$date2
&&ideni=$ideni&&remain=$amount' title='UKO YISHYUYE'>
 $ideni </A> </td>
<td title='UKO YISHYUWE' style='padding: 1px; padding-left: 25px; padding-left: 5px; border-top-width: 1px; border-bottom-width: 1px; font-weight: bold'> $ampayf</td>
...
<tr><td title="UKO YISHYUWE" style="padding: 3px; padding-right: 25px; padding-left: 5px; border-top-width: 1px; border-bottom-width: 1px; font-weight: bold">AMADENI YOSE YISHYUWE</td>
<td style="padding: 3px; border-top-width: 1px; border-bottom-width: 1px; white-space: nowrap; text-align: right; font-weight: bold">
<?php $totampayf=number_format($totampay); echo"$totampayf";?></td>
</tr>

<tr><td style="padding: 3px; padding-right: 25px; padding-left: 5px; border-top-width: 1px; border-bottom-width: 1px; font-weight: bold">AMADENI YOSE ASIGAYE</td>
<td style="padding: 3px; border-top-width: 1px; border-bottom-width: 1px; white-space: nowrap; text-align: right; font-weight: bold">
<?php $totcreditf=number_format($totcredit); echo"$totcreditf";?></td>
</tr>
<tr><td colspan="2">&nbsp;</td></tr></tbody><tfoot><tr><td style="text-align: center; padding-top: 20px" colspan="2"><?php echo"LIABILITIES REPORT";?></td>
</tr></tfoot><td> 
</table>
