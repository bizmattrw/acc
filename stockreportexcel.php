              ...
              <div class="panel-heading">
                <div class="row">
				  ...
				  <?PHP $date1=$_GET['date1'];$date2=$_GET['date2']; $item=$_GET['item'];?>
                    <h3 class="panel-title"><font color="#FFFFFF"></font>THE DETAILS ON STOCK: 
...
header("Content-type: application/vnd-ms-excel");
 header("Content-Disposition: attachment; filename=FICHESTOCK.xls");
...
$id=$med['id'];
 $action=$med['action'];
$sel1=mysqli_query($con,"select quantityadded as am from stock where action='credit' and id='$id' order by date,id asc");
$med1=mysqli_fetch_array($sel1);
$amountdebit=$med1['am'];
$sel2=mysqli_query($con,"select quantityremoved as amcredit from stock where action='debit'  and id='$id'  order by date,id asc") or die(mysqli_error($con));
$med2=mysqli_fetch_array($sel2);
...
<td align=center> $amountcreditf</td><td align=center> $sprice</td><td align=center> $valuerf</td><td align=center> $balancef</td><td align=center> $sprice</td><td align=center> $valuebf</td>
...
echo"<tr><th colspan=2>TOTALS</TH><th>$totqaf</th><th></th><th>$totvaf</th><th>$totqrf</th><th></th><th>$totvrf</th></tr>";
  ...
  ?>
    ...
    <br /><?php
$sel1=mysqli_query($con,"select sum(quantityadded) as debit from stock where  date>='$date1' AND date<='$date2' and action='credit' and item='$item' and coopid='$_SESSION[coopid]' group by action") or die(mysqli_error($con));
$med1=mysqli_fetch_array($sel1);
$debit=$med1['debit'];
$sel2=mysqli_query($con,"select sum(quantityremoved) as credit from stock where  date>='$date1' AND date<='$date2' and action='debit' and item='$item' and coopid='$_SESSION[coopid]' group by action") or die(mysqli_error($con));
$med2=mysqli_fetch_array($sel2);
$credit=$med2['credit'];
$z1=$debit-$credit;
$z2=$z1+$z;
